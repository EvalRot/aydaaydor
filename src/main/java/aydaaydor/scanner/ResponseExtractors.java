...
/**
 * Utilities to extract normalized maps of significant values from HTTP
 * responses (JSON, HTML, XML).
 */
final class ResponseExtractors {
    ...
    static BodyType detectBodyType(HttpResponse resp, String body) {
        // custom method for detecting response body type
        try {
    ...
    static Map<String, String> extractSignificant(HttpResponse resp, String body, Collection<String> ignoredJsonKeys) {
        // the method to extract significant values from HTTP responses. JSON, HTML, XML
        // elements are extracted
        // from response body and are used then to build a map of baseline values to use
        // for comparison while testing
        // for other IDs further
    ...
    private static void collectJson(String path, JsonElement el, Map<String, String> out, Set<String> ignored) {
        // the Depth-first search to recursively collect JSON elements from the response
        // body and create a map of baseline values
        ...
        if (el.isJsonObject()) {
            ...
            String val;
            if (p.isBoolean())
                val = String.valueOf(p.getAsBoolean());
            else if (p.isNumber())
                val = String.valueOf(p.getAsNumber());
            else
                val = p.getAsString();
            String n = norm(val);
            if (n != null)
